...
const double e = 0.05, critical_value_kolmogorov = 1.36, critical_value_pirson =  16.92, k = 10;
...
double Pirson(vector<double> a)
{
	...
	while (i <= 1)
	{
		...
		X += (((double)cnt - (double)N / (double)k) * ((double)cnt - (double)N / (double)k) / ((double)N / (double)k));	
		i += (1.0 / (double)k);
		cnt = 0;
	}
	return X;
